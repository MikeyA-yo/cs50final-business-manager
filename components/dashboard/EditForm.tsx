...
export default function EditInvoice({ id }: { id: string }) {
  ...
  return (
    <>
      <div
        className={`flex min-h-screen items-center justify-center w-full ${mont.className}`}
      >
        <form
          className="flex flex-col gap-2 bg-[#EBF4F6] p-4 shadow-md shadow-[#37B7C3]"
          onSubmit={(e) => {
            e.preventDefault();
          }}
        >
          ...
          {invoice && (
            <>
              <label htmlFor="amount">
                Edit Amount, current amount: {invoice.amount}
              </label>
              <input
                name="amount"
              ...
              <button
                type="submit"
                className="bg-[#088395] p-2 flex gap-1 items-center justify-center"
                onClick={() => {
                  updateInvoice();
                }}
              >
